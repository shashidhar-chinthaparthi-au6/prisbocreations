...
export async function POST(req: Request) {
  ...
  if (file.size > MAX_BYTES) {
    return jsonError("Maximum file size is 5 MB", 400);
  }

  const buf = Buffer.from(await file.arrayBuffer());
  const meta = await sharp(buf).rotate().metadata();
  let pipeline =
    meta.width && meta.height && (meta.width > MAX_DIM || meta.height > MAX_DIM)
  ...
  if (cfg) {
    const key = `uploads/reviews/${fileId}${ext}`;
  ...
  const url = `${publicOrigin(req)}/uploads/reviews/${diskName}`;
  return jsonOk({ url, fileId, filename });
}
